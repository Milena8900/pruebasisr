...
function theme_breadcrumbs_items($args) {
    ...
    if (!is_front_page()) {
        $items[] = theme_breadcrumbs_link($args, get_home_url(), '', __('Home', 'pruebasisr'));
    }
            ...
            if (get_post_type() === 'product') {
                ...
                if ($terms) {
                    foreach (array_reverse($terms) as $term) {
                        ...
                        $product_name = get_term_by('id', $product_cat_id, 'product_cat', 'ARRAY_A')['name'];
    ...
    if (is_tag()) {
        $items[] = theme_breadcrumbs_text($args, __("Tag: ", 'pruebasisr') . single_tag_title('', FALSE));
    }
    if (is_404()) {
        $items[] = theme_breadcrumbs_text($args, __("404 - Page not Found", 'pruebasisr'));
    }
    if (is_search()) {
        $items[] = theme_breadcrumbs_text($args, __("Search", 'pruebasisr'));
    }
    if (is_year()) {
        $items[] = theme_breadcrumbs_text($args, get_the_time('Y'));
    }
    if (is_author()) {
        $items[] = theme_breadcrumbs_text($args, sprintf(esc_attr(__('View all posts by %s', 'pruebasisr')), get_the_author()));
    }
